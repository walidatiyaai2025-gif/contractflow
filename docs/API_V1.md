...
Mutation response semantics are explicit: malformed/unsupported input is HTTP 422, missing capability/scope is HTTP 403, archived-domain mutation conflicts are HTTP 409, and unexpected server errors remain generic 500 responses without internal diagnostics.
...
REST suites under `tests/php/rest_api_*.php` validate the P8 read/version/security contract. `tests/php/rest_mobile_mutations_013_019.php` validates the narrow P9 mutation routes, capability gates, fail-closed field allow-lists, conflict semantics and domain-service delegation. All are executed by `scripts/test-php.sh` and repository Quality Gates.

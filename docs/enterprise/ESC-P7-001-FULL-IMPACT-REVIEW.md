...
Replacement starts one transaction and locks the exact current-tenant Transition joined to its active Workflow, active Contract Type, and exact draft Workflow Version. Existing selectors, stages, and route identity are removed only within that transaction. New route/stage/selector rows are then persisted from the canonical server-normalized policy.

Every `tenant_user` selector is revalidated under lock against an active membership in the same active tenant before the stage/selector write completes. An inactive/foreign membership after a route row has been inserted causes rollback, proving that partial configuration is not committed.

Published Workflow Version route definitions are immutable.

## Workflow publication impact

P6-001 publication remains the single one-way draft-to-published operation. Inside the existing publication transaction, after graph validation and P6-004 guard validation, P7-001 now revalidates stored Approval Route data before the Workflow Version row can be marked published.
...
The validator rebuilds policy input from stored rows and runs the same canonical `ApprovalRoutePolicy`, so authoring and publication do not have divergent rule sets.
...
Definition replacement is serialized on the exact draft Transition and is fully transactional. It does not create runtime request identity or decision idempotency yet; those belong to later P7 runtime tasks.

Publication runs under the existing locked draft Workflow Version transaction and locks the Approval Route rows/stages/selectors it validates, preventing configuration from being changed between validation and publication.
...
Implementation head: `b471ce2a53ee1fb52e623d5196a0dca1bc43a450`.

ESC Foundation Gate #405 passed:
...
- P7-001: 58/58 assertions;
- Flutter formatting, analysis and tests.

The P7-001 regression covers schema/version registration, canonical policy, bounds, duplicate rejection, `all`/`quorum`, tenant role constraints, active same-tenant user selection, atomic rollback on membership failure, published immutability, stale publication snapshots, invalid roles/quorum at publication, authorization ceiling, publication ordering and explicit absence of P7 runtime dependencies from P6 transition execution.

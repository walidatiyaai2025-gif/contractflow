...
final class WorkbookReader
{
    ...
    private const MAX_UNCOMPRESSED_BYTES = 67108864; // 64 MiB
    private const MAX_SHEETS = 32;
    private const MAX_COLUMNS = 128;
    private const MAX_ROWS = 50000;

    /** @return array{sheets:list<array{name:string,path:string,header_row:int,headers:list<array{column:string,original:string,normalized:string}>}>} */
    public function discover(string $path): array
    {
        ...
        try {
            ...
            foreach ($this->sheetDefinitions($zip) as $definition) {
                $rows = $this->sheetRows($zip, $definition['path'], $shared, 20);
                $header = $this->firstHeader($rows);
                $sheets[] = [
    ...
    /** @return list<array{row_number:int,cells:array<string,string>}> */
    ...
    private function sheetDefinitions(ZipArchive $zip): array
    {
        ...
        foreach ($relXpath->query('//r:Relationship') ?: [] as $node) {
            ...
            $targets[$node->getAttribute('Id')] = $node->getAttribute('Target');
        }

        $doc = $this->document($workbook);
        ...
        foreach ($xpath->query('//s:sheets/s:sheet') ?: [] as $node) {
            ...
            $target = (string) ($targets[$relationshipId] ?? '');
            ...
            if ($name === '' || ! str_starts_with($path, 'xl/worksheets/') || str_contains($path, '../')) {
                continue;
            }
            $definitions[] = ['name' => substr($name, 0, 191), 'path' => $path];
            if (count($definitions) > self::MAX_SHEETS) {
                throw new InvalidArgumentException('Workbook contains too many worksheets.');
            }
        }
        return $definitions;
    }
    ...
    private function sharedStrings(ZipArchive $zip): array
    {
        ...
        foreach ($xpath->query('//s:si') ?: [] as $item) {
            ...
            if (count($values) > 100000) {
                throw new InvalidArgumentException('Workbook shared string table is too large.');
            }
        }
        return $values;
    }
    ...
    /** @return array{row_number:int,headers:list<array{column:string,original:string,normalized:string}>} */
    private function firstHeader(array $rows): array
    {
        foreach ($rows as $row) {
            ...
            if ($headers !== []) {
                return ['row_number' => (int) $row['row_number'], 'headers' => $headers];
            }
        }
    ...
    private function cellValue(DOMXPath $xpath, DOMElement $cell, array $shared): string
    {
        ...
        if ($type === 'inlineStr') {
            $parts = [];
            foreach ($xpath->query('.//s:is//s:t', $cell) ?: [] as $text) {
                $parts[] = $text->textContent;
            }
    ...
    private function xml(ZipArchive $zip, string $path): string
    {
        ...
        if (! is_string($xml) || $xml === '' || strlen($xml) > 16777216) {
            throw new InvalidArgumentException('Workbook XML part is missing or too large.');
        }
        if (stripos($xml, '<!DOCTYPE') !== false || stripos($xml, '<!ENTITY') !== false) {
            throw new InvalidArgumentException('Workbook XML declarations are not allowed.');
        }
        return $xml;
    }
    ...
    private function document(string $xml): DOMDocument
    {
        ...
        try {
            ...
            if (! $doc->loadXML($xml, LIBXML_NONET | LIBXML_COMPACT)) {
                throw new InvalidArgumentException('Workbook XML is invalid.');
            }
            return $doc;
        } finally {
            libxml_clear_errors();

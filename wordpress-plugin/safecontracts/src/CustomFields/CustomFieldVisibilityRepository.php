...
final class CustomFieldVisibilityRepository
{
    private const RULE_COLUMNS = 'id, target_definition_id, contract_type_id, match_mode, target_field_code_snapshot, target_data_type_snapshot, target_config_hash, created_by, updated_by, created_at, updated_at';
    ...
    public function findRule(int $targetDefinitionId): ?array
    {
        ...
        $table = $wpdb->prefix . 'safecontracts_custom_field_visibility_rules';
    ...
    public function listConditions(int $ruleId): array
    {
        global $wpdb;
        $tenantId = $this->tenantId();
        $conditions = $wpdb->prefix . 'safecontracts_custom_field_visibility_conditions';
        $definitions = $wpdb->prefix . 'safecontracts_custom_field_definitions';
        $rows = $wpdb->get_results($wpdb->prepare(
            "SELECT c.id, c.rule_id, c.target_definition_id, c.position_no, c.source_definition_id, c.operator_code, c.operand_json,
            ...
            $tenantId,
            $ruleId,
            CustomFieldVisibilityPolicy::MAX_CONDITIONS + 1
        ), ARRAY_A);
    ...
    public function listDependencyEdges(int $contractTypeId, int $limit = self::MAX_GRAPH_SCAN): array
    {
        global $wpdb;
        $tenantId = $this->tenantId();
        $rules = $wpdb->prefix . 'safecontracts_custom_field_visibility_rules';
        $conditions = $wpdb->prefix . 'safecontracts_custom_field_visibility_conditions';
        $limit = max(1, min(self::MAX_GRAPH_SCAN, $limit));
        $rows = $wpdb->get_results($wpdb->prepare(
            "SELECT r.target_definition_id, c.source_definition_id
             FROM {$rules} r
             INNER JOIN {$conditions} c ON c.rule_id = r.id AND c.tenant_id = r.tenant_id
     ...
     * @param list<array<string,mixed>> $conditionsData
     */
    public function replaceRule(array $target, string $matchMode, array $conditionsData, int $actorId): void
    {
        global $wpdb;
        $tenantId = $this->tenantId();
        $rules = $wpdb->prefix . 'safecontracts_custom_field_visibility_rules';
        ...
        if ($wpdb->query('START TRANSACTION') === false) {
            throw new RuntimeException('Unable to start Dynamic Field visibility rule transaction.');
        }

        try {
            $definitionIds = [$targetId];
            foreach ($conditionsData as $condition) {
                $definitionIds[] = (int) ($condition['source_definition_id'] ?? 0);
            }
            ...
            if ($definitionIds === [] || in_array(0, $definitionIds, true)) {
                throw new RuntimeException('Dynamic Field visibility rule contains an invalid definition identity.');
            }
            ...
            $lockedRows = $wpdb->get_results($wpdb->prepare(
                "SELECT id FROM {$definitions}
                 WHERE tenant_id = %d AND contract_type_id = %d AND status = 'active' AND id IN ({$placeholders})
            ...
            if ($lockedIds !== $definitionIds) {
                throw new RuntimeException('Dynamic Field visibility definitions changed concurrently or are no longer active in the Contract Type.');
            }

            $ruleSql = $wpdb->prepare(
                "INSERT INTO {$rules} (tenant_id, target_definition_id, contract_type_id, match_mode, target_field_code_snapshot, target_data_type_snapshot, target_config_hash, created_by, updated_by, created_at, updated_at)
                 ...
                 ON DUPLICATE KEY UPDATE id = LAST_INSERT_ID(id), contract_type_id = VALUES(contract_type_id), match_mode = VALUES(match_mode),
                    target_field_code_snapshot = VALUES(target_field_code_snapshot), target_data_type_snapshot = VALUES(target_data_type_snapshot),
                    target_config_hash = VALUES(target_config_hash), updated_by = VALUES(updated_by), updated_at = UTC_TIMESTAMP()",
                $tenantId,
                $matchMode,
            ...
            if ($ruleResult === false) {
                throw new RuntimeException('Unable to persist Dynamic Field visibility rule.');
            }
            ...
            if ($ruleId <= 0) {
                throw new RuntimeException('Dynamic Field visibility rule persistence returned no identifier.');
            }

            $deleteResult = $wpdb->query($wpdb->prepare(
                "DELETE FROM {$conditions} WHERE tenant_id = %d AND rule_id = %d AND target_definition_id = %d",
                $tenantId,
                $ruleId,
                $targetId
            ));
            if ($deleteResult === false) {
                throw new RuntimeException('Unable to replace Dynamic Field visibility conditions.');
            }

            foreach ($conditionsData as $condition) {
                $source = $condition['source_definition'];
                $operand = $condition['operand_json'];
                $operandSql = $operand === null ? 'NULL' : $wpdb->prepare('%s', $operand);
                ...
                $insertSql = $wpdb->prepare(
                    "INSERT INTO {$conditions} (tenant_id, rule_id, target_definition_id, position_no, source_definition_id, operator_code, operand_json, source_field_code_snapshot, source_data_type_snapshot, source_config_hash, created_by, updated_by, created_at, updated_at)
                     SELECT %d, %d, %d, %d, d.id, %s, {$operandSql}, %s, %s, %s, %d, %d, UTC_TIMESTAMP(), UTC_TIMESTAMP()
                     FROM {$definitions} d
                     WHERE d.id = %d AND d.tenant_id = %d AND d.contract_type_id = %d AND d.status = 'active'
                       AND d.field_code = %s AND d.data_type = %s
                       AND COALESCE(d.options_json, '') = %s AND COALESCE(d.validation_json, '') = %s",
                    ...
                    $ruleId,
                    $targetId,
                    (int) $condition['position_no'],
                    (string) $condition['operator_code'],
                ...
                if ($insertResult === false) {
                    throw new RuntimeException('Unable to persist Dynamic Field visibility condition.');
                }
                if ($insertResult === 0) {
                    throw new RuntimeException('Dynamic Field visibility source definition changed concurrently.');
                }
            }

            if ($wpdb->query('COMMIT') === false) {
                throw new RuntimeException('Unable to commit Dynamic Field visibility rule transaction.');
            }
    ...
    public function resetRule(array $target): void
    {
        ...
        if ($wpdb->query('START TRANSACTION') === false) {
            throw new RuntimeException('Unable to start Dynamic Field visibility reset transaction.');
        }
        try {
            $locked = $wpdb->get_results($wpdb->prepare(
                "SELECT d.id FROM {$definitions} d
                 WHERE d.id = %d AND d.tenant_id = %d AND d.contract_type_id = %d AND d.status = 'active' AND d.data_type = %s
                 LIMIT 1 FOR UPDATE",
                $targetId,
                $tenantId,
                $contractTypeId,
            ...
            if (! is_array($locked) || $locked === []) {
                throw new RuntimeException('Dynamic Field visibility target changed concurrently or is no longer resettable.');
            }
            ...
            if ($ruleId > 0) {
                if ($wpdb->query($wpdb->prepare("DELETE FROM {$conditions} WHERE tenant_id = %d AND rule_id = %d", $tenantId, $ruleId)) === false) {
                    throw new RuntimeException('Unable to delete Dynamic Field visibility conditions.');
                }
                if ($wpdb->query($wpdb->prepare("DELETE FROM {$rules} WHERE tenant_id = %d AND id = %d AND target_definition_id = %d", $tenantId, $ruleId, $targetId)) === false) {
                    throw new RuntimeException('Unable to reset Dynamic Field visibility rule.');
                }
            }
            if ($wpdb->query('COMMIT') === false) {
                throw new RuntimeException('Unable to commit Dynamic Field visibility reset transaction.');
            }
        } catch (\Throwable $error) {
            $wpdb->query('ROLLBACK');
            throw $error;
    ...
    public function listValues(int $contractId, array $definitionIds): array
    {
        ...
        if (count($definitionIds) > CustomFieldVisibilityPolicy::MAX_CONDITIONS) {
            throw new RuntimeException('Dynamic Field visibility value request exceeds the bounded condition limit.');
        }

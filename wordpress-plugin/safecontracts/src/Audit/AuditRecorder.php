...
final class AuditRecorder
{
    ...
    private static function map(string $hook, array $args): array
    {
        return match ($hook) {
            ...
            'safecontracts_contract_counterparty_assigned' => [
                'contract', (int) ($args[0] ?? 0), 'contract_counterparty_assigned', (int) ($args[4] ?? 0),
                ['counterparty_type' => $args[5] ?? null, 'counterparty_id' => $args[6] ?? null],
                ['counterparty_type' => $args[1] ?? null, 'counterparty_id' => $args[2] ?? null, 'financial_direction' => $args[3] ?? null],
                null,
            ],
            'safecontracts_contract_currency_changed' => [
                'contract', (int) ($args[0] ?? 0), 'contract_currency_changed', (int) ($args[3] ?? 0),
                ['currency_code' => $args[1] ?? null], ['currency_code' => $args[2] ?? null], null,
            ],
            'safecontracts_contract_accountant_assigned' => [
            ...
            'safecontracts_supplier_created' => [
                'supplier', (int) ($args[0] ?? 0), 'supplier_created', (int) ($args[1] ?? 0), null, ['created' => true], null,
            ],
            'safecontracts_supplier_updated' => [
                'supplier', (int) ($args[0] ?? 0), 'supplier_updated', (int) ($args[1] ?? 0), null, ['updated' => true], null,
            ],
            'safecontracts_supplier_archived' => [
                'supplier', (int) ($args[0] ?? 0), 'supplier_archived', (int) ($args[1] ?? 0), null,
                ['is_archived' => true], ['had_contract_history' => (bool) ($args[2] ?? false)],
            ],

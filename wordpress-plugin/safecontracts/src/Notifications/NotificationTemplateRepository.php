...
final class NotificationTemplateRepository
{
    ...
    public function findByCode(string $code, bool $activeOnly = true): ?array
    {
        ...
        $active = $activeOnly ? ' AND is_active = 1' : '';
        $rows = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT id, code, name, title_template, body_template, is_active, created_by, updated_by, created_at, updated_at
                 FROM {$table} WHERE code = %s{$active} LIMIT 1",
                $code
            ),
            ARRAY_A
        );
    ...
    public function save(array $template, int $actorId): void
    {
        ...
        $wpdb->query($wpdb->prepare(
            "INSERT INTO {$table}
             ...
             ON DUPLICATE KEY UPDATE name = VALUES(name), title_template = VALUES(title_template),

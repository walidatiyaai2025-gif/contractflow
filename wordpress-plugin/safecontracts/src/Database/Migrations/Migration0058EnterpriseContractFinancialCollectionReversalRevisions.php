...
final class Migration0058EnterpriseContractFinancialCollectionReversalRevisions implements Migration
{
    public function up(object $wpdb): void
    {
        ...
        $revisions = $wpdb->prefix . 'safecontracts_contract_financial_collection_reversal_revisions';
        ...
        dbDelta("CREATE TABLE {$revisions} (
            ...
            PRIMARY KEY  (id),
            UNIQUE KEY financial_collection_reversal_revision_uuid (revision_uuid),
            UNIQUE KEY tenant_contract_collection_reversal_revision (tenant_id, contract_id, reversal_uuid, revision_number),
            KEY tenant_contract_collection_reversal_latest (tenant_id, contract_id, reversal_uuid, revision_number, id),
            KEY tenant_contract_collection_reversal_receipt (tenant_id, contract_id, receipt_uuid, reversal_state, id),
            KEY tenant_contract_collection_reversal_date_state (tenant_id, contract_id, reversal_date, reversal_state, id),
            KEY tenant_contract_collection_reversal_profile (tenant_id, contract_id, financial_currency_profile_id)
        ) {$charset};");
    }
}

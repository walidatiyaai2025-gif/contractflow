...
final class ContractFinancialPaymentScheduleRevisionService
{
    public function __construct(private ?ContractFinancialPaymentScheduleRevisionRepository $repository = null)
    {
        $this->repository ??= new ContractFinancialPaymentScheduleRevisionRepository();
    }
    ...
    /** @return array{id:int,schedule_entry_uuid:string} */
    public function create(int $contractId, mixed $sequenceNo, mixed $reference, mixed $dueDate, mixed $amount): array
    {
        $this->assertContractId($contractId);
        $this->authorize(Capabilities::MANAGE_PAYMENTS);
        $entryUuid = $this->uuid();
        $id = $this->repository->createEntry(
            $contractId,
            $entryUuid,
            $this->uuid(),
            ContractFinancialPaymentSchedulePolicy::normalizeSequence($sequenceNo),
            ContractFinancialPaymentSchedulePolicy::normalizeReference($reference),
            ContractFinancialPaymentSchedulePolicy::normalizeDueDate($dueDate),
            $amount,
            get_current_user_id(),
        ...
        return ['id' => $id, 'schedule_entry_uuid' => $entryUuid];
    }

    public function revise(int $contractId, mixed $entryUuid, mixed $reference, mixed $dueDate, mixed $amount): int
    {
        $this->assertContractId($contractId);
        $this->authorize(Capabilities::MANAGE_PAYMENTS);
        return $this->repository->reviseEntry(
            $contractId,
            ContractFinancialPaymentSchedulePolicy::normalizeUuid($entryUuid, 'schedule entry UUID'),
            $this->uuid(),
            ContractFinancialPaymentSchedulePolicy::normalizeReference($reference),
            ContractFinancialPaymentSchedulePolicy::normalizeDueDate($dueDate),
            $amount,
    ...
    public function void(int $contractId, mixed $entryUuid): int
    {
        $this->assertContractId($contractId);
        $this->authorize(Capabilities::MANAGE_PAYMENTS);
        return $this->repository->voidEntry(
            $contractId,
            ContractFinancialPaymentSchedulePolicy::normalizeUuid($entryUuid, 'schedule entry UUID'),
            $this->uuid(),
            get_current_user_id(),
    ...
    private function authorize(string $capability): void
    {
        ...
        if (! current_user_can($capability) || ! TenantAuthorization::allowsCapability($capability)) {
            throw new DomainException('The current tenant role does not allow this Enterprise Contract payment schedule operation.');
        }
    ...
    private function uuid(): string
    {
        ...
        try {
            ...
            throw new RuntimeException('Unable to generate Enterprise Contract payment schedule UUID.', 0, $error);
        }

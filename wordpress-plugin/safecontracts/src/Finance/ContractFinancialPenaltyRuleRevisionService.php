...
final class ContractFinancialPenaltyRuleRevisionService
{
    public function __construct(private ?ContractFinancialPenaltyRuleRevisionRepository $repository = null)
    {
        $this->repository ??= new ContractFinancialPenaltyRuleRevisionRepository();
    }
    ...
    /** @return array{id:int,penalty_rule_uuid:string} */
    public function create(int $contractId, mixed $label, mixed $mode, mixed $configuredValue): array
    {
        $this->assertContractId($contractId);
        ...
        $id = $this->repository->createRule(
            ...
            $ruleUuid,
            $this->uuid(),
            ContractFinancialPenaltyRulePolicy::normalizeLabel($label),
            ContractFinancialPenaltyRulePolicy::normalizeMode($mode),
            $configuredValue,
            get_current_user_id(),
            function (array $contract): void {
                $this->assertScope($contract);
        ...
        return ['id' => $id, 'penalty_rule_uuid' => $ruleUuid];
    }

    public function revise(int $contractId, mixed $ruleUuid, mixed $label, mixed $mode, mixed $configuredValue): int
    {
        $this->assertContractId($contractId);
        $this->authorize(Capabilities::EDIT_CONTRACTS);
        return $this->repository->reviseRule(
            $contractId,
            ContractFinancialPenaltyRulePolicy::normalizeUuid($ruleUuid, 'penalty rule UUID'),
            $this->uuid(),
            ContractFinancialPenaltyRulePolicy::normalizeLabel($label),
            ContractFinancialPenaltyRulePolicy::normalizeMode($mode),
            $configuredValue,
    ...
    private function authorize(string $capability): void
    {
        ...
        if (! current_user_can($capability) || ! TenantAuthorization::allowsCapability($capability)) {
            throw new DomainException('The current tenant role does not allow this Enterprise Contract penalty rule operation.');
        }
    ...
    private function uuid(): string
    {
        ...
        try {
            ...
            throw new RuntimeException('Unable to generate Enterprise Contract penalty rule UUID.', 0, $error);
        }

...
final class ContractFinancialRetentionRuleRevisionService
{
    public function __construct(private ?ContractFinancialRetentionRuleRevisionRepository $repository = null)
    {
        $this->repository ??= new ContractFinancialRetentionRuleRevisionRepository();
    }
    ...
    /** @return array{id:int,retention_rule_uuid:string} */
    public function create(int $contractId, mixed $label, mixed $ratePercent): array
    {
        $this->assertContractId($contractId);
        ...
        $id = $this->repository->createRule(
            ...
            $ruleUuid,
            $this->uuid(),
            ContractFinancialRetentionRulePolicy::normalizeLabel($label),
        ...
        return ['id' => $id, 'retention_rule_uuid' => $ruleUuid];
    }

    public function revise(int $contractId, mixed $ruleUuid, mixed $label, mixed $ratePercent): int
    {
        $this->assertContractId($contractId);
        $this->authorize(Capabilities::EDIT_CONTRACTS);
        return $this->repository->reviseRule(
            $contractId,
            ContractFinancialRetentionRulePolicy::normalizeUuid($ruleUuid, 'retention rule UUID'),
            $this->uuid(),
            ContractFinancialRetentionRulePolicy::normalizeLabel($label),
            PercentageRate::of($ratePercent),
            get_current_user_id(),
    ...
    private function authorize(string $capability): void
    {
        ...
        if (! current_user_can($capability) || ! TenantAuthorization::allowsCapability($capability)) {
            throw new DomainException('The current tenant role does not allow this Enterprise Contract retention rule operation.');
        }
    ...
    private function uuid(): string
    {
        ...
        try {
            ...
            throw new RuntimeException('Unable to generate Enterprise Contract retention rule UUID.', 0, $error);
        }

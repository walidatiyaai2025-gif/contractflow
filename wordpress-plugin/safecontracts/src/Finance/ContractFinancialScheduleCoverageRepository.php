...
final class ContractFinancialScheduleCoverageRepository
{
     ...
     * @return array{
     *   financial:array{
     *     profile:array{id:int,currency:string},
     *     base:array{amount:string,currency:string,profile_id:int,revision_number:int},
     *     adjustments:list<array{line_uuid:string,revision_number:int,kind:string,amount:string,currency:string,state:string,profile_id:int}>
     *   },
     *   schedules:list<array{schedule_entry_uuid:string,sequence_no:int,amount:string,currency:string,state:string}>
     * }
     */
    public function snapshot(int $contractId, callable $authorizeLockedContract): array
    {
        if ($contractId <= 0) {
            throw new RuntimeException('Enterprise schedule coverage requires a positive Contract identifier.');
        }
        ...
        $scheduleRevisions = $wpdb->prefix . 'safecontracts_contract_financial_payment_schedule_entry_revisions';

        if ($wpdb->query('START TRANSACTION') === false) {
            throw new RuntimeException('Unable to start Enterprise schedule coverage snapshot transaction.');
        }
        ...
        try {
            ...
            if (! is_array($profileRows) || count($profileRows) !== 1 || ! is_array($profileRows[0])) {
                throw new RuntimeException('Enterprise schedule coverage requires exactly one Contract financial currency profile.');
            }
            ...
            if (! is_array($baseRows) || count($baseRows) !== 1 || ! is_array($baseRows[0])) {
                throw new RuntimeException('Enterprise schedule coverage requires an explicit Contract base-value revision.');
            }
            $base = $baseRows[0];
            ...
            if (! $baseMoney->currency()->equals($profileCurrency) || $baseMoney->compare(Money::of('0', $profileCurrency)) < 0) {
                throw new UnexpectedValueException('Enterprise Contract base value differs from its profile or is negative.');
            }

            $adjustmentLimit = ContractFinancialAdjustmentPolicy::MAX_LINES + 1;
            ...
            if (! is_array($adjustmentRows)) {
                throw new RuntimeException('Enterprise schedule coverage adjustment snapshot could not be read.');
            }
            if (count($adjustmentRows) > ContractFinancialAdjustmentPolicy::MAX_LINES) {
                throw new RuntimeException('Enterprise schedule coverage adjustment limit was exceeded.');
            }
            ...
            foreach ($adjustmentRows as $row) {
                if (! is_array($row)) {
                    throw new UnexpectedValueException('Enterprise schedule coverage adjustment row is invalid.');
                }
                try {
                    ...
                    throw new UnexpectedValueException('Enterprise schedule coverage contains invalid adjustment metadata.', 0, $error);
                }
                ...
                if ($rowId <= 0 || $rowContractId !== $contractId || $rowProfileId !== $profileId
                    ...
                    throw new UnexpectedValueException('Enterprise schedule coverage adjustment identity/profile metadata is invalid.');
                }
                if (isset($seenLines[$lineUuid])) {
                    throw new UnexpectedValueException('Enterprise schedule coverage contains duplicate latest adjustment identities.');
                }
                $seenLines[$lineUuid] = true;

                $money = $this->money($row['amount'] ?? null, $row['currency_code'] ?? null, 'financial adjustment');
                if (! $money->currency()->equals($profileCurrency) || $money->compare(Money::of('0', $profileCurrency)) < 0) {
                    throw new UnexpectedValueException('Enterprise schedule coverage adjustment currency or amount is invalid.');
                }
                $adjustments[] = [
                    'line_uuid' => $lineUuid,
            ...
            $scheduleRows = $wpdb->get_results($wpdb->prepare(
                "SELECT r.id, r.revision_uuid, r.contract_id, r.financial_currency_profile_id, r.schedule_entry_uuid,
                        r.revision_number, r.sequence_no, r.reference, r.due_date, r.amount,
                        r.currency_code, r.schedule_entry_state, r.created_by
                 FROM {$scheduleRevisions} r
                 WHERE r.tenant_id = %d AND r.contract_id = %d
                   AND NOT EXISTS (
                        SELECT 1 FROM {$scheduleRevisions} newer
                        WHERE newer.tenant_id = r.tenant_id
                          AND newer.contract_id = r.contract_id
            ...
            if (! is_array($scheduleRows)) {
                throw new RuntimeException('Enterprise schedule coverage payment schedule snapshot could not be read.');
            }
            if (count($scheduleRows) > ContractFinancialPaymentSchedulePolicy::MAX_ENTRIES) {
                throw new RuntimeException('Enterprise schedule coverage payment schedule limit was exceeded.');
            }
            ...
            foreach ($scheduleRows as $row) {
                if (! is_array($row)) {
                    throw new UnexpectedValueException('Enterprise schedule coverage payment schedule row is invalid.');
                }
                try {
                    ContractFinancialPaymentSchedulePolicy::normalizeUuid($row['revision_uuid'] ?? null, 'revision UUID');
                    $scheduleUuid = ContractFinancialPaymentSchedulePolicy::normalizeUuid($row['schedule_entry_uuid'] ?? null, 'schedule entry UUID');
                    $sequence = ContractFinancialPaymentSchedulePolicy::normalizeSequence($row['sequence_no'] ?? null);
                    ContractFinancialPaymentSchedulePolicy::normalizeReference($row['reference'] ?? null);
                    ContractFinancialPaymentSchedulePolicy::normalizeDueDate($row['due_date'] ?? null);
                    $state = ContractFinancialPaymentSchedulePolicy::normalizeState($row['schedule_entry_state'] ?? null);
                } catch (Throwable $error) {
                    throw new UnexpectedValueException('Enterprise schedule coverage contains invalid payment schedule metadata.', 0, $error);
                }
                ...
                $createdBy = (int) ($row['created_by'] ?? 0);
                if ($rowId <= 0 || $rowContractId !== $contractId || $rowProfileId !== $profileId
                    || $revisionNumber <= 0 || $revisionNumber > ContractFinancialPaymentSchedulePolicy::MAX_REVISION || $createdBy <= 0) {
                    throw new UnexpectedValueException('Enterprise schedule coverage payment schedule identity/profile metadata is invalid.');
                }
                if (isset($seenScheduleUuids[$scheduleUuid])) {
                    throw new UnexpectedValueException('Enterprise schedule coverage contains duplicate latest payment schedule identities.');
                }
                if (isset($seenSequences[$sequence])) {
                    throw new UnexpectedValueException('Enterprise schedule coverage contains duplicate latest payment schedule sequence numbers.');
                }
                $seenScheduleUuids[$scheduleUuid] = true;
                $seenSequences[$sequence] = true;

                $money = $this->money($row['amount'] ?? null, $row['currency_code'] ?? null, 'payment schedule amount');
                if (! $money->currency()->equals($profileCurrency) || $money->compare(Money::of('0', $profileCurrency)) <= 0) {
                    throw new UnexpectedValueException('Enterprise schedule coverage payment schedule currency or amount is invalid.');
                }
                $schedules[] = [
            ...
            if ($wpdb->query('COMMIT') === false) {
                throw new RuntimeException('Unable to commit Enterprise schedule coverage snapshot transaction.');
            }

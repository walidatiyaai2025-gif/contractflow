...
final class ContractFinancialCreditRevisionService
{
    public function __construct(private ?ContractFinancialCreditRevisionRepository $repository = null)
    {
        $this->repository ??= new ContractFinancialCreditRevisionRepository();
    }
    ...
    /** @return array{id:int,credit_uuid:string} */
    public function create(int $contractId, mixed $reason, mixed $amount): array
    {
        $this->assertContractId($contractId);
        $this->authorize(Capabilities::EDIT_CONTRACTS);
        $creditUuid = $this->uuid();
        $id = $this->repository->createCredit(
            $contractId,
            $creditUuid,
            $this->uuid(),
            ContractFinancialCreditPolicy::normalizeReason($reason),
            $amount,
            get_current_user_id(),
        ...
        return ['id' => $id, 'credit_uuid' => $creditUuid];
    }

    public function revise(int $contractId, mixed $creditUuid, mixed $reason, mixed $amount): int
    {
        $this->assertContractId($contractId);
        $this->authorize(Capabilities::EDIT_CONTRACTS);
        return $this->repository->reviseCredit(
            $contractId,
            ContractFinancialCreditPolicy::normalizeUuid($creditUuid, 'credit UUID'),
            $this->uuid(),
            ContractFinancialCreditPolicy::normalizeReason($reason),
            $amount,
            get_current_user_id(),
    ...
    public function void(int $contractId, mixed $creditUuid): int
    {
        $this->assertContractId($contractId);
        $this->authorize(Capabilities::EDIT_CONTRACTS);
        return $this->repository->voidCredit(
            $contractId,
            ContractFinancialCreditPolicy::normalizeUuid($creditUuid, 'credit UUID'),
            $this->uuid(),
            get_current_user_id(),
    ...
    private function authorize(string $capability): void
    {
        ...
        if (! current_user_can($capability) || ! TenantAuthorization::allowsCapability($capability)) {
            throw new DomainException('The current tenant role does not allow this Enterprise Contract credit operation.');
        }
    ...
    private function uuid(): string
    {
        ...
        try {
            ...
            throw new RuntimeException('Unable to generate Enterprise Contract credit UUID.', 0, $error);
        }

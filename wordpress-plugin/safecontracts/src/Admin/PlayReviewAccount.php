...
final class PlayReviewAccount
{
    ...
    public static function registerPage(): void
    {
        add_submenu_page(
            AdminShell::SLUG,
            __('Google Play Review', 'safecontracts'),
            __('Google Play Review', 'safecontracts'),
    ...
    public static function renderPage(): void
    {
        self::requireManage();
        $status = self::status();
        $notice = isset($_GET['safecontracts_status']) && is_string($_GET['safecontracts_status'])
            ? sanitize_key($_GET['safecontracts_status'])
            : '';
        $messages = [
        ...
        <div class="wrap safecontracts-settings" dir="auto">
            ...
            <?php if ($notice !== '' && isset($messages[$notice])): ?>
                <div class="notice <?php echo esc_attr(in_array($notice, ['review_ready', 'review_disabled'], true) ? 'notice-success' : 'notice-error'); ?> is-dismissible"><p><?php echo esc_html($messages[$notice]); ?></p></div>
            <?php endif; ?>
            <section class="safecontracts-admin-card safecontracts-settings-card">
                ...
                <table class="widefat striped" style="max-width:760px"><tbody>
                    ...
                    <tr><th><?php echo esc_html__('WordPress login', 'safecontracts'); ?></th><td><code><?php echo esc_html(self::LOGIN); ?></code> <?php echo esc_html__('(email login also works in WordPress)', 'safecontracts'); ?></td></tr>
                ...
                <p class="description"><?php echo esc_html__('Enter the exact temporary password you want to give Google Play. For security, the plugin does not embed a fixed plaintext password in the public repository.', 'safecontracts'); ?></p>
                <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="max-width:760px">
                    ...
                    <p class="description"><?php echo esc_html__('If you choose a weak password such as 123456, keep this account enabled only for the shortest review window and disable it immediately after Google finishes reviewing the app.', 'safecontracts'); ?></p>
                ...
                <?php if ($status['managed']): ?>
                    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                        ...
                        <?php wp_nonce_field(self::DISABLE_ACTION); ?>
                        <?php submit_button(__('Disable Review Account', 'safecontracts'), 'secondary'); ?>
                    </form>
                <?php endif; ?>

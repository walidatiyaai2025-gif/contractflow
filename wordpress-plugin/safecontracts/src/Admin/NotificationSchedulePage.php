...
final class NotificationSchedulePage
{
    ...
    public static function register(): void
    {
        add_submenu_page(
            AdminShell::SLUG,
            __('Notification Schedule', 'safecontracts'),
            __('Notification Schedule', 'safecontracts'),
    ...
    public static function render(): void
    {
        ...
        $status = isset($_GET['notification_status']) && is_scalar($_GET['notification_status'])
            ? sanitize_key((string) $_GET['notification_status'])
            : '';
        ...
        $rows = empty($filters['date_range_error'])
            ? $repository->recent($filters['date_from'], $filters['date_to'], $status, 300)
            : [];
        ...
        <div class="wrap safecontracts-settings" dir="auto">
            ...
            <section class="safecontracts-admin-card">
                <div class="safecontracts-field-row">
                    <form method="get" action="<?php echo esc_url(admin_url('admin.php')); ?>">
                        ...
                        <label><?php echo esc_html__('Status', 'safecontracts'); ?>
                            <select name="notification_status">
                                <option value=""><?php echo esc_html__('All', 'safecontracts'); ?></option>
                                <?php foreach (NotificationScheduleRepository::statuses() as $option) : ?>
                                    <option value="<?php echo esc_attr($option); ?>" <?php selected($status, $option); ?>><?php echo esc_html(self::stateLabel($option)); ?></option>
                                <?php endforeach; ?>
                            </select>
                        </label>
                    ...
                    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                        <input type="hidden" name="action" value="<?php echo esc_attr(self::SAVE_TIME_ACTION); ?>">
                        <?php wp_nonce_field(self::SAVE_TIME_ACTION); ?>
                        <label><?php echo esc_html__('Daily dispatch time', 'safecontracts'); ?>
                            <input type="time" name="dispatch_time" required value="<?php echo esc_attr($settings->dispatchTime()); ?>">
                        </label>
                        <?php submit_button(__('Save time', 'safecontracts'), 'secondary', '', false); ?>
                    </form>
                </div>
                <?php if (! empty($filters['date_range_error'])) : ?>
                    <p class="notice notice-error inline"><strong><?php echo esc_html__('Invalid period: the From date must not be after the To date.', 'safecontracts'); ?></strong></p>
                <?php endif; ?>
                <p class="description">
                    <?php echo esc_html__('Schedule dates follow the configured WordPress/site timezone. Dispatch runs every five minutes through WP-Cron; actual execution depends on site traffic or the server cron that invokes WordPress cron.', 'safecontracts'); ?>
                    <?php if ($lastRun !== '') : ?> <?php echo esc_html__('Last scheduler run (UTC):', 'safecontracts'); ?> <code><?php echo esc_html($lastRun); ?></code><?php endif; ?>
                </p>
            </section>
            ...
            <section class="safecontracts-admin-card safecontracts-table-card">
                ...
                <table class="widefat striped">
                    <thead><tr>
                        <th><?php echo esc_html__('Scheduled', 'safecontracts'); ?></th>
                        <th><?php echo esc_html__('Notification', 'safecontracts'); ?></th>
                        <th><?php echo esc_html__('Recipients / result', 'safecontracts'); ?></th>
                        <th><?php echo esc_html__('Sent via', 'safecontracts'); ?></th>
                        <th><?php echo esc_html__('State', 'safecontracts'); ?></th>
                        <th><?php echo esc_html__('Last attempt', 'safecontracts'); ?></th>
                        <th><?php echo esc_html__('Action', 'safecontracts'); ?></th>
                    </tr></thead>
                    <tbody>
                    <?php if ($rows === []) : ?>
                        <tr><td colspan="7"><?php echo esc_html__('No scheduled notifications match this period and status.', 'safecontracts'); ?></td></tr>
                    <?php endif; ?>
                    <?php foreach ($rows as $row) : ?>
                        <?php
                        $outcomes = $deliveries->outcomesForOccurrence((int) $row['rule_id'], (int) $row['payment_id'], (string) $row['scheduled_date'], (int) $row['attempt_no']);
                        ...
                        <tr>
                            ...
                            <td>
                                <strong><?php echo esc_html((string) ($row['rule_name'] ?? $row['rule_code'] ?? '')); ?></strong><br>
                                <?php echo esc_html__('Contract', 'safecontracts'); ?>: <code><?php echo esc_html((string) ($row['contract_number'] ?? '')); ?></code><br>
                                <?php echo esc_html__('Customer', 'safecontracts'); ?>: <?php echo esc_html((string) ($row['customer_name'] ?? '')); ?><br>
                                <?php echo esc_html__('Payment', 'safecontracts'); ?>: #<?php echo esc_html((string) $row['payment_id']); ?><?php if (! empty($row['payment_reference'])) : ?> · <?php echo esc_html((string) $row['payment_reference']); ?><?php endif; ?><br>
                                <small><?php echo esc_html__('Rule attempt', 'safecontracts'); ?>: <?php echo esc_html((string) $row['attempt_no']); ?></small>
                            </td>
                            <td><?php self::renderRecipients($recipientIds, $outcomes); ?></td>
                            <td><strong><?php echo esc_html__('Push', 'safecontracts'); ?></strong><br><small>Firebase Cloud Messaging</small></td>
                            <td>
                                <strong><?php echo esc_html(self::stateLabel((string) $row['status'])); ?></strong><br>
                                <small><?php echo esc_html(sprintf(__('Sent %d / Failed %d / Recipients %d', 'safecontracts'), (int) $row['sent_count'], (int) $row['failed_count'], (int) $row['recipient_count'])); ?></small>
                                <?php if (! empty($row['last_error_code'])) : ?><br><code><?php echo esc_html((string) $row['last_error_code']); ?></code><?php endif; ?>
                            </td>
                            ...
                            <td>
                                <?php if ((string) $row['status'] === 'processing') : ?>
                                    <button class="button" disabled><?php echo esc_html__('Sending…', 'safecontracts'); ?></button>
                                <?php else : ?>
                                    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" onsubmit="return confirm('<?php echo esc_js(__('Send this notification now using the current rule, recipients and Firebase configuration?', 'safecontracts')); ?>');">
                                        ...
                                        <?php wp_nonce_field(self::MANUAL_SEND_ACTION . '_' . (int) $row['id']); ?>
                                        <button type="submit" class="button button-secondary"><?php echo esc_html((string) $row['status'] === 'sent' ? __('Resend manually', 'safecontracts') : __('Send manually', 'safecontracts')); ?></button>
                                    </form>
                                <?php endif; ?>
                            </td>
                        </tr>
                ...
                <p class="description"><?php echo esc_html__('Manual Send never bypasses settled-payment suppression, active-rule checks, recipient resolution, Firebase transport, delivery logging or audit recording.', 'safecontracts'); ?></p>
            </section>
        </div>
        <?php
    }
    ...
    private static function renderRecipients(array $recipientIds, array $outcomes): void
    {
        ...
        foreach ($recipientIds as $userId) {
            ...
            if (is_array($outcome) && ! empty($outcome['error_code'])) {
                echo ' <code>' . esc_html((string) $outcome['error_code']) . '</code>';
            }
            echo '</div>';
        }
    }

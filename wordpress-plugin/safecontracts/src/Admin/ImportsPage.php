...
final class ImportsPage
{
        ...
        <div class="wrap safecontracts-settings" dir="auto">
            ...
            <section class="safecontracts-admin-card safecontracts-settings-card">
                ...
                <form method="post" enctype="multipart/form-data" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                    ...
                    <p><input type="file" name="workbook" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" required></p>
                    <?php submit_button(__('Upload & inspect workbook', 'safecontracts')); ?>
                </form>
            </section>

            <section class="safecontracts-admin-card safecontracts-table-card">
                <h2><?php echo esc_html__('Recent import runs', 'safecontracts'); ?></h2>
                <table class="widefat striped"><thead><tr><th><?php echo esc_html__('Run', 'safecontracts'); ?></th><th><?php echo esc_html__('Workbook', 'safecontracts'); ?></th><th><?php echo esc_html__('Status', 'safecontracts'); ?></th><th><?php echo esc_html__('Created', 'safecontracts'); ?></th></tr></thead><tbody>
                <?php foreach ($recent as $item) : ?>
                    <tr><td><a href="<?php echo esc_url(add_query_arg(['page' => self::SLUG, 'run_id' => (int) $item['id']], admin_url('admin.php'))); ?>">#<?php echo esc_html((string) $item['id']); ?></a></td><td><?php echo esc_html((string) $item['original_filename']); ?></td><td><?php echo esc_html((string) $item['status']); ?></td><td><?php echo esc_html((string) $item['created_at']); ?></td></tr>
                <?php endforeach; ?>
                </tbody></table>
            </section>
            ...
            <?php if ($run !== null) : ?>
                <section class="safecontracts-admin-card safecontracts-settings-card">
                    <h2><?php echo esc_html(sprintf(__('Run #%d — column mapping', 'safecontracts'), (int) $run['id'])); ?></h2>
                    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                        <input type="hidden" name="action" value="<?php echo esc_attr(self::MAP_ACTION); ?>"><input type="hidden" name="run_id" value="<?php echo esc_attr((string) $run['id']); ?>">
                        <?php wp_nonce_field(self::MAP_ACTION); ?>
                        <p><label><?php echo esc_html__('Worksheet', 'safecontracts'); ?><select class="widefat" name="selected_sheet" required>
                            <?php foreach ($run['discovery']['sheets'] ?? [] as $sheet) : ?><option value="<?php echo esc_attr((string) $sheet['name']); ?>" <?php selected((string) ($run['selected_sheet'] ?? ''), (string) $sheet['name']); ?>><?php echo esc_html((string) $sheet['name']); ?></option><?php endforeach; ?>
                        </select></label></p>
                        <div class="safecontracts-role-grid">
                        <?php
                        $selectedSheet = null;
                        $selectedName = (string) ($run['selected_sheet'] ?? (($run['discovery']['sheets'][0]['name'] ?? '')));
                        if ($selectedName !== '') {
                            try { $selectedSheet = ColumnMapping::sheet($run['discovery'], $selectedName); } catch (Throwable $error) { unset($error); }
                        }
                        $headers = is_array($selectedSheet['headers'] ?? null) ? $selectedSheet['headers'] : [];
                        foreach (ColumnMapping::fields() as $field => $definition) :
                        ?>
                            <label><?php echo esc_html($definition['label'] . ($definition['required'] ? ' *' : '')); ?><select class="widefat" name="mapping[<?php echo esc_attr($field); ?>]"><option value=""><?php echo esc_html__('Ignore / not mapped', 'safecontracts'); ?></option><?php foreach ($headers as $header) : ?><option value="<?php echo esc_attr((string) $header['column']); ?>" <?php selected((string) ($run['mapping'][$field] ?? ''), (string) $header['column']); ?>><?php echo esc_html((string) $header['column'] . ' — ' . (string) $header['original']); ?></option><?php endforeach; ?></select></label>
                        <?php endforeach; ?>
                        </div>
                        <?php submit_button(__('Save mapping', 'safecontracts')); ?>
                    </form>
                </section>

                <?php if ($run['mapping'] !== []) : ?>
                <section class="safecontracts-admin-card safecontracts-table-card">
                    <h2><?php echo esc_html__('Import preview', 'safecontracts'); ?></h2>
                    <?php if ($previewError !== '') : ?><p><?php echo esc_html($previewError); ?></p><?php elseif ($preview === []) : ?><p><?php echo esc_html__('No data rows found after the header.', 'safecontracts'); ?></p><?php else : ?>
                    <table class="widefat striped"><thead><tr><th><?php echo esc_html__('Row', 'safecontracts'); ?></th><?php foreach (array_keys($run['mapping']) as $field) : ?><th><?php echo esc_html($field); ?></th><?php endforeach; ?></tr></thead><tbody>
                    <?php foreach ($preview as $row) : ?><tr><td><?php echo esc_html((string) $row['row_number']); ?></td><?php foreach (array_keys($run['mapping']) as $field) : ?><td><?php echo esc_html((string) ($row['data'][$field] ?? '')); ?></td><?php endforeach; ?></tr><?php endforeach; ?>
                    </tbody></table>
                    <?php endif; ?>
                    <p class="description"><?php echo esc_html__('Preview is read-only. Business data is not mutated until explicit import execution after validation.', 'safecontracts'); ?></p>
                </section>
                <?php endif; ?>
            <?php endif; ?>
        </div>
    ...
    {
        if (! is_scalar($value) || is_bool($value)) {
            throw new \InvalidArgumentException('Import field must be scalar text.');
        }
        $text = trim(strip_tags((string) $value));
        if ($text === '' || strlen($text) > $max) {
            throw new \InvalidArgumentException('Import field is empty or too long.');
        }
        return $text;
    }
}

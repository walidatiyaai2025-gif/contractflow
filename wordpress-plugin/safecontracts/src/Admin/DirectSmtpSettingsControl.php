...
final class DirectSmtpSettingsControl
{
        ...
        <div class="notice notice-info" style="padding: 14px 18px; border-inline-start-width: 4px;">
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" autocomplete="off">
                ...
                <table class="form-table" role="presentation" style="margin-top: 0;">
                    <tr>
                        <th scope="row"><label for="safecontracts-smtp-host"><?php echo esc_html(self::text('SMTP host', 'خادم SMTP')); ?></label></th>
                        <td><input id="safecontracts-smtp-host" class="regular-text" name="host" maxlength="253" required value="<?php echo esc_attr($settings['host']); ?>" placeholder="smtp.example.com"></td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="safecontracts-smtp-port"><?php echo esc_html(self::text('SMTP port', 'منفذ SMTP')); ?></label></th>
                        <td><input id="safecontracts-smtp-port" type="number" min="1" max="65535" name="port" required value="<?php echo esc_attr((string) $settings['port']); ?>"></td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="safecontracts-smtp-encryption"><?php echo esc_html(self::text('Encryption', 'التشفير')); ?></label></th>
                        <td><select id="safecontracts-smtp-encryption" name="encryption">
                            <option value="tls" <?php selected($settings['encryption'], 'tls'); ?>>STARTTLS / TLS</option>
                            <option value="ssl" <?php selected($settings['encryption'], 'ssl'); ?>>SSL / TLS</option>
                            <option value="none" <?php selected($settings['encryption'], 'none'); ?>><?php echo esc_html(self::text('None', 'بدون')); ?></option>
                        </select></td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="safecontracts-smtp-username"><?php echo esc_html(self::text('SMTP username', 'اسم مستخدم SMTP')); ?></label></th>
                        <td><input id="safecontracts-smtp-username" class="regular-text" name="username" maxlength="191" value="<?php echo esc_attr($settings['username']); ?>" autocomplete="off"><p class="description"><?php echo esc_html(self::text('Leave username and password empty only if the SMTP server explicitly allows unauthenticated relay.', 'اترك اسم المستخدم وكلمة المرور فارغين فقط إذا كان خادم SMTP يسمح صراحة بالإرسال بدون مصادقة.')); ?></p></td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="safecontracts-smtp-password"><?php echo esc_html(self::text('SMTP password', 'كلمة مرور SMTP')); ?></label></th>
                        <td><input id="safecontracts-smtp-password" class="regular-text" type="password" name="password" maxlength="1024" value="" autocomplete="new-password" placeholder="<?php echo esc_attr($settings['password_configured'] ? self::text('Saved — leave blank to keep', 'محفوظة — اتركها فارغة للاحتفاظ بها') : ''); ?>">
                            <?php if ($settings['password_configured']) : ?><p><label><input type="checkbox" name="clear_password" value="1"> <?php echo esc_html(self::text('Clear saved SMTP password', 'حذف كلمة مرور SMTP المحفوظة')); ?></label></p><?php endif; ?>
                            <p class="description"><?php echo esc_html(self::text('The SMTP password is encrypted before it is stored in the WordPress database and is never displayed back in this form.', 'يتم تشفير كلمة مرور SMTP قبل حفظها في قاعدة بيانات ووردبريس ولا يتم عرضها مرة أخرى في هذه الصفحة.')); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="safecontracts-smtp-timeout"><?php echo esc_html(self::text('Connection timeout', 'مهلة الاتصال')); ?></label></th>
                        <td><input id="safecontracts-smtp-timeout" type="number" min="3" max="60" name="timeout" required value="<?php echo esc_attr((string) $settings['timeout']); ?>"> <?php echo esc_html(self::text('seconds', 'ثانية')); ?></td>
                    </tr>
                </table>
    ...
    public static function filterLegacyMailCopy(string $translation, string $text, string $domain = 'safecontracts'): string
    {
        ...
        return self::text(
            'Safe Contracts sends email through the Direct SMTP connection configured above. WordPress wp_mail and WordPress SMTP plugins are not used.',
            'يرسل Safe Contracts البريد من خلال اتصال SMTP المباشر المضبوط أعلاه. لا يتم استخدام wp_mail أو إضافات SMTP الخاصة بووردبريس.'
        );
    }

...
final class MutationController
{
    ...
    public static function editContract(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        try {
            ...
            if ($hasStart && $hasEnd) {
                $service->updateDates(
                    $contractId,
                    self::nullableScalar($input['start_date'], 'start_date'),
                    self::nullableScalar($input['end_date'], 'end_date')
                );
            }
    ...
    public static function editPaymentExpectedDate(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        try {
            ...
            $expected = self::nullableScalar($input['expected_payment_date'], 'expected_payment_date');
            $service->updateDates($paymentId, $payment['due_date'], $expected);

            return RequestGuard::response([
                'id' => $paymentId,
                'updated' => true,
                'expected_payment_date' => $expected === null ? null : trim((string) $expected),
            ]);
        } catch (InvalidArgumentException $error) {
            return RequestGuard::invalid($error, 'safecontracts_payment_light_edit_invalid');
        } catch (DomainException $error) {
            return RequestGuard::domain($error, 'safecontracts_payment_light_edit_forbidden');
        } catch (Throwable $error) {
    ...
    public static function recordCollection(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        try {
            ...
            return RequestGuard::domain($error, 'safecontracts_collection_record_forbidden');
        } catch (Throwable $error) {
    ...
    public static function recordFollowUp(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        try {
            ...
            $note = array_key_exists('note', $input) ? self::nullableScalar($input['note'], 'note') : null;
            $promisedDate = array_key_exists('promised_date', $input)
                ? self::nullableScalar($input['promised_date'], 'promised_date')
                : null;
            $deferredUntil = array_key_exists('deferred_until', $input)
                ? self::nullableScalar($input['deferred_until'], 'deferred_until')
                : null;
            ...
            return RequestGuard::domain($error, 'safecontracts_followup_record_forbidden');
        } catch (Throwable $error) {

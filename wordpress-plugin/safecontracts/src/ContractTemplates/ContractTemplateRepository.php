...
final class ContractTemplateRepository
{
    ...
    public function publishDraftVersion(int $templateId, int $versionId, int $actorId): void
    {
        ...
        $table = $wpdb->prefix . 'safecontracts_contract_template_versions';
        $sql = $wpdb->prepare(
            "UPDATE {$table} SET version_status = 'published', published_by = %d, published_at = UTC_TIMESTAMP(), updated_by = %d, updated_at = UTC_TIMESTAMP()
             WHERE id = %d AND template_id = %d AND tenant_id = %d AND version_status = 'draft'",
            $actorId,
            $actorId,
            $versionId,
        ...
        if ($result === 0) {
            throw new RuntimeException('Contract Template version changed concurrently or is no longer publishable.');
        }
    }

...
// SC-P6-001 — SafeContracts admin shell.
$GLOBALS['sc_test_current_caps'] = [];
do_action('admin_menu');
$shell = $GLOBALS['sc_test_admin_pages'][AdminShell::SLUG] ?? null;
sc_p6a_assert(is_array($shell), 'SC-P6-001 top-level SafeContracts admin page is registered');
sc_p6a_assert(($shell['capability'] ?? null) === Capabilities::ACCESS, 'SC-P6-001 shell visibility is capability-gated');
sc_p6a_assert(($shell['icon'] ?? null) === 'dashicons-shield-alt', 'SC-P6-001 shell uses SafeContracts security-oriented menu identity');
sc_p6a_assert(is_callable($shell['callback'] ?? null), 'SC-P6-001 shell callback is registered');

sc_p6a_expect(RuntimeException::class, [AdminShell::class, 'render'], 'SC-P6-001 shell render rejects users without SafeContracts access');
$GLOBALS['sc_test_current_caps'] = [Capabilities::ACCESS => true];
ob_start();
AdminShell::render();
$rendered = (string) ob_get_clean();
sc_p6a_assert(str_contains($rendered, 'SafeContracts'), 'SC-P6-001 shell renders SafeContracts identity');
...
sc_p6a_assert(is_array($adminStyle) && str_ends_with((string) ($adminStyle['src'] ?? ''), 'assets/admin/safecontracts-admin.css'), 'SC-P6-001 shell loads dedicated SafeContracts admin identity stylesheet');
$_GET['page'] = 'safecontracts-payment-methods';
sc_p6a_assert(AdminShell::isSafeContractsPage(), 'SC-P6-001 existing SafeContracts subpages inherit shell styling boundary');
...
sc_p6a_assert(is_string($adminCss) && str_contains($adminCss, '--safecontracts-navy') && str_contains($adminCss, '--safecontracts-green'), 'SC-P6-001 admin stylesheet carries navy/green SafeContracts identity tokens');
...
sc_p6a_assert(str_contains(LoginBranding::headerText('WordPress'), 'SafeContracts'), 'SC-P6-002 login header text uses SafeContracts identity');
$loginCss = file_get_contents(dirname(__DIR__, 2) . '/wordpress-plugin/safecontracts/assets/admin/safecontracts-login.css');
sc_p6a_assert(is_string($loginCss) && str_contains($loginCss, 'body.login') && str_contains($loginCss, 'content: "SC"'), 'SC-P6-002 login CSS replaces visual WordPress identity with SafeContracts mark');
...
sc_p6a_assert(! in_array(AdminShell::SLUG, $removed, true), 'SC-P6-003 SafeContracts entry point is never removed');
...
NavigationCleanup::cleanup();
sc_p6a_assert($GLOBALS['sc_test_removed_admin_menus'] === [], 'SC-P6-003 SafeContracts system administrator is exempt from menu cleanup');

$GLOBALS['sc_test_removed_admin_menus'] = [];
$GLOBALS['sc_test_current_caps'] = [];
NavigationCleanup::cleanup();
sc_p6a_assert($GLOBALS['sc_test_removed_admin_menus'] === [], 'SC-P6-003 non-SafeContracts users are not modified by SafeContracts navigation cleanup');

add_filter('safecontracts_hidden_admin_menus', static fn (mixed $menus): array => ['index.php', AdminShell::SLUG]);
$GLOBALS['sc_test_removed_admin_menus'] = [];
$GLOBALS['sc_test_current_caps'] = [Capabilities::ACCESS => true];
NavigationCleanup::cleanup();
sc_p6a_assert($GLOBALS['sc_test_removed_admin_menus'] === ['index.php'], 'SC-P6-003 filtered cleanup remains extensible but refuses to remove SafeContracts shell');
...
echo "SafeContracts P6 admin UI SC-P6-001..003 passed ({$tests} assertions).\n";

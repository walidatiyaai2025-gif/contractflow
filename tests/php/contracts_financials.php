...
$activate();
sc_fin_assert(Migrator::LATEST_VERSION === '1.6.0', 'payment schedule migration is current after financial/history schema');
sc_fin_assert(count($GLOBALS['sc_test_dbdelta']) === 9, 'contract financial schemas remain present before history/payment schemas');
...
sc_fin_assert(count($GLOBALS['sc_test_dbdelta']) === $dbDeltaCount, 'financial, history and payment migrations are idempotent after stored version is current');

...
sc_p11f_assert($filters['direction'] === FinancialDirection::PAYABLE, 'finance direction normalizes');
...
sc_p11f_assert(FinanceReadAccess::authorizedDirections() === [], 'VIEW_ALL alone does not grant finance direction access');

$GLOBALS['sc_test_current_caps'] = [
    Capabilities::VIEW_ALL => true,
    Capabilities::VIEW_PAYABLES => true,
];
sc_p11f_assert(FinanceReadAccess::authorizedDirections() === [FinancialDirection::PAYABLE], 'payable permission exposes only AP');
sc_p11f_expect(
    DomainException::class,
    fn () => FinanceReadAccess::resolveDirections(FinancialDirection::RECEIVABLE),
    'AP-only user cannot request AR'
);
...
$GLOBALS['sc_test_current_caps'] = [
    ...
    Capabilities::VIEW_RECEIVABLES => true,
];
sc_p11f_assert(FinanceReadAccess::authorizedDirections() === [FinancialDirection::RECEIVABLE], 'receivable permission exposes only AR');
...
sc_p11f_assert(str_contains($sqlScope['where'], "c.counterparty_type = 'customer'"), 'customer filter is explicit counterparty semantics');
sc_p11f_assert(str_contains($sqlScope['where'], 'p.currency_code = %s'), 'currency is a real server-side filter');
...
$GLOBALS['sc_test_result_queue'] = [[
    [
        ...
        'currency_code' => 'KWD',
        'obligation_count' => '2',
        'original_total' => '10000.0000',
        'settled_total' => '3000.0000',
        'outstanding_total' => '7000.0000',
        'overdue_total' => '2000.0000',
        'overdue_count' => '1',
        'due_today_total' => '0.0000',
        'due_today_count' => '0',
        'due_7_total' => '1000.0000',
        'due_7_count' => '1',
        'due_30_total' => '5000.0000',
        'due_30_count' => '1',
    ...
    [
        ...
        'currency_code' => 'USD',
        'obligation_count' => '1',
        'original_total' => '20000.0000',
        'settled_total' => '5000.0000',
        'outstanding_total' => '15000.0000',
        'overdue_total' => '0.0000',
        'overdue_count' => '0',
        'due_today_total' => '0.0000',
        'due_today_count' => '0',
        'due_7_total' => '0.0000',
        'due_7_count' => '0',
        'due_30_total' => '15000.0000',
        'due_30_count' => '1',
...
sc_p11f_assert(str_contains($summarySql, "'payable', 'receivable'"), 'authorized AP and AR directions are explicitly bounded in SQL');
...
sc_p11f_assert(str_contains($queueSql, "c.counterparty_type = 'supplier'"), 'Supplier lookup is type-aware');
...
$controllerSource = file_get_contents(dirname(__DIR__, 2) . '/wordpress-plugin/safecontracts/src/Rest/FinanceController.php');
$dashboardSource = file_get_contents(dirname(__DIR__, 2) . '/wordpress-plugin/safecontracts/src/Rest/DashboardController.php');
sc_p11f_assert(is_string($controllerSource) && str_contains($controllerSource, "'/finance/overview'"), 'finance overview REST endpoint is registered');
sc_p11f_assert(is_string($controllerSource) && str_contains($controllerSource, "'/finance/obligations'"), 'finance obligation REST endpoint is registered');
sc_p11f_assert(is_string($controllerSource) && ! str_contains($controllerSource, 'Capabilities::VIEW_ALL)\n            ||'), 'VIEW_ALL is not used as finance authorization bypass');
...
sc_p11f_assert($empty['directions'] === [] && $empty['summary'] === [], 'dashboard finance section fails closed without finance view permissions');

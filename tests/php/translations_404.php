...
sc_i18n_assert(RuntimeLabels::default('Pending') === 'قيد المتابعة', 'runtime follow-up state has Arabic default');
sc_i18n_assert(RuntimeLabels::default('Assigned Accountant') === 'المحاسب المسند', 'runtime recipient label has Arabic default');
...
sc_i18n_assert(isset($catalog['Pending']), 'catalog discovers runtime enum label hints');
...
TranslationCatalog::saveRows([
    ...
    ['source' => 'Pending', 'en' => 'Awaiting action', 'ar' => 'بانتظار الإجراء'],
]);
...
sc_i18n_assert(RuntimeLabels::text('Pending') === 'بانتظار الإجراء', 'dashboard override wins over runtime Arabic default');
$GLOBALS['sc_test_locale'] = 'en_US';
sc_i18n_assert(RuntimeLabels::text('Pending') === 'Awaiting action', 'dashboard override wins over runtime English source');
...
sc_i18n_assert(RuntimeLabels::filterGettext('Pending', 'Pending', 'safecontracts') === 'قيد المتابعة', 'Arabic runtime gettext fallback works');
$GLOBALS['sc_test_locale'] = 'en_US';
...
sc_i18n_assert(RuntimeLabels::filterGettext('Pending', 'Pending', 'safecontracts') === 'Pending', 'runtime filter does not alter English');
...
sc_i18n_assert(str_contains($plugin, 'RuntimeLabels::register()'), 'plugin registers runtime enum translation filter');
...
sc_i18n_assert(str_contains($translationsPage, 'RuntimeLabels::default'), 'translation editor shows runtime Arabic defaults');
...
$singleLanguageFiles = [
    ...
    'wordpress-plugin/safecontracts/src/Admin/CustomersPage.php',
    'wordpress-plugin/safecontracts/src/Admin/ContractsPage.php',
    'wordpress-plugin/safecontracts/src/Admin/PaymentsPage.php',
    'wordpress-plugin/safecontracts/src/Admin/CollectionsPage.php',
    'wordpress-plugin/safecontracts/src/Admin/PaymentMethodsPage.php',
];
foreach ($singleLanguageFiles as $relative) {
    $source = sc_i18n_source($relative);
    foreach ([' / حذف', ' / فتح', ' / الإجراءات', ' / العقود النشطة'] as $mixedMarker) {
        sc_i18n_assert(! str_contains($source, $mixedMarker), 'admin surface has no hard-coded mixed-language suffix: ' . $relative . ' :: ' . $mixedMarker);
    }
}
...
$translationSources = $plugin
    ...
    . sc_i18n_source('wordpress-plugin/safecontracts/src/Translations/RuntimeLabels.php')
    . $translationsPage;

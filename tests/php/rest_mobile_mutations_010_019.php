...
// Direct callback defense-in-depth: denial occurs before body validation or domain I/O.
$deniedFollowUp = MutationController::recordFollowUp(new WP_REST_Request(['payment_id' => '21']));
sc_p9m_assert($deniedFollowUp instanceof WP_Error && ($deniedFollowUp->data['status'] ?? 0) === 403, 'SC-P9-019 direct callback rechecks capability before parsing mutation');
...
sc_p9m_assert(MutationController::canEditContracts() === true, 'SC-P9-013 capability + scoped access permits contract light edit');
sc_p9m_assert(MutationController::canManagePayments() === true, 'SC-P9-016 capability + scoped access permits payment light edit');
...
sc_p9m_assert($badFollowUp instanceof WP_Error && ($badFollowUp->data['status'] ?? 0) === 422, 'SC-P9-019 promise requires an explicit promised date');

$source = file_get_contents(dirname(__DIR__, 2) . '/wordpress-plugin/safecontracts/src/Rest/MutationController.php') ?: '';
sc_p9m_assert(! str_contains($source, '$wpdb'), 'SC-P9-013/016/017/019 REST mutation boundary contains no direct SQL');
sc_p9m_assert(str_contains($source, 'new ContractService()'), 'SC-P9-013 delegates contract mutation to ContractService');
sc_p9m_assert(str_contains($source, 'new PaymentService()'), 'SC-P9-016 delegates payment mutation to PaymentService');
sc_p9m_assert(str_contains($source, 'new CollectionService()'), 'SC-P9-017 delegates collection mutation to CollectionService');
sc_p9m_assert(str_contains($source, 'new FollowUpService()'), 'SC-P9-019 delegates follow-up mutation to FollowUpService');

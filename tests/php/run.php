...
sc_assert(str_contains($paymentSchema, 'wp_safecontracts_scheduled_payments'), 'scheduled-payment table created');
sc_assert(str_contains($paymentSchema, 'original_amount decimal(20,4) NOT NULL'), 'scheduled payment amount uses fixed-point precision');
sc_assert(str_contains($paymentSchema, 'due_date date NOT NULL'), 'scheduled payment stores required contractual due date');
sc_assert(str_contains($paymentSchema, 'expected_payment_date date NULL'), 'scheduled payment stores separate expected date');
sc_assert(str_contains($paymentSchema, 'UNIQUE KEY contract_sequence (contract_id, sequence_no)'), 'payment sequence is unique within contract');

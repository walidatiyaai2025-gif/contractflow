...
esc_p5_meta_assert(str_contains($schema, 'show_in_form tinyint(1) NOT NULL DEFAULT 1'), 'form visibility has explicit default');
esc_p5_meta_assert(str_contains($schema, 'show_in_mobile tinyint(1) NOT NULL DEFAULT 1'), 'mobile visibility has explicit default');
esc_p5_meta_assert(str_contains($schema, 'filterable tinyint(1) NOT NULL DEFAULT 0'), 'report filtering is disabled by default');
esc_p5_meta_assert(str_contains($schema, 'exportable tinyint(1) NOT NULL DEFAULT 0'), 'report export is disabled by default');
...
esc_p5_meta_assert($textDefaults['show_in_form'] === true && $textDefaults['show_in_mobile'] === true, 'operational form/mobile visibility defaults are deterministic');
esc_p5_meta_assert($textDefaults['show_in_summary'] === false && $textDefaults['show_in_print'] === false, 'secondary presentation surfaces default closed');
esc_p5_meta_assert($textDefaults['filterable'] === false && $textDefaults['sortable'] === false && $textDefaults['exportable'] === false, 'reporting eligibility defaults closed');
esc_p5_meta_assert($textDefaults['report_data_class'] === 'text' && $textDefaults['aggregation_policy'] === 'none', 'text reporting defaults are deterministic');
esc_p5_meta_assert(CustomFieldMetadataPolicy::defaults('decimal')['report_data_class'] === 'measure', 'numeric fields default to measure class without aggregation execution');
esc_p5_meta_assert(CustomFieldMetadataPolicy::defaults('date')['report_data_class'] === 'date', 'date fields default to date reporting class');
esc_p5_meta_assert(count(CustomFieldMetadataPolicy::reportDataClasses()) === 5, 'report data-class allowlist is bounded');
esc_p5_meta_assert(count(CustomFieldMetadataPolicy::aggregationPolicies()) === 5, 'aggregation policy allowlist is bounded');
...
esc_p5_meta_assert($numeric['report_label'] === 'Amount' && $numeric['aggregation_policy'] === 'sum', 'numeric reporting metadata normalizes deterministically');
esc_p5_meta_throws(static fn () => CustomFieldMetadataPolicy::normalize('text', ['report_data_class' => 'measure']), InvalidArgumentException::class, 'text field cannot masquerade as numeric measure');
esc_p5_meta_throws(static fn () => CustomFieldMetadataPolicy::normalize('date', ['aggregation_policy' => 'sum']), InvalidArgumentException::class, 'date field cannot aggregate numerically');
esc_p5_meta_throws(static fn () => CustomFieldMetadataPolicy::normalize('decimal', ['report_data_class' => 'text']), InvalidArgumentException::class, 'numeric field rejects incompatible text report class');
...
esc_p5_meta_throws(static fn () => CustomFieldMetadataPolicy::normalize('multi_select', ['groupable' => true]), InvalidArgumentException::class, 'multi-select cannot claim ambiguous groupable eligibility');
esc_p5_meta_throws(static fn () => CustomFieldMetadataPolicy::normalize('text', ['show_in_form' => 1]), InvalidArgumentException::class, 'visibility flags require JSON booleans');
...
esc_p5_meta_throws(static fn () => CustomFieldMetadataPolicy::normalize('text', ['expression' => 'value > 10']), InvalidArgumentException::class, 'unsupported expression property fails closed');
...
esc_p5_meta_assert(($defaults['show_in_form'] ?? false) === true && ($defaults['filterable'] ?? true) === false, 'default metadata is conservative and deterministic');
esc_p5_meta_assert($GLOBALS['sc_test_queries'] === [], 'metadata default read performs no mutation');
esc_p5_meta_assert(str_contains($GLOBALS['sc_test_read_queries'][1] ?? '', 'WHERE id = 61 AND tenant_id = 17'), 'definition read is locked-tenant scoped');
esc_p5_meta_assert(str_contains($GLOBALS['sc_test_read_queries'][2] ?? '', 'WHERE tenant_id = 17 AND definition_id = 61'), 'metadata read is locked-tenant scoped');

$GLOBALS['sc_test_queries'] = [];
$GLOBALS['sc_test_read_queries'] = [];
$GLOBALS['sc_test_result_queue'] = [esc_p5_meta_actor(), esc_p5_meta_definition(), []];
$noOp = $service->upsert(61, []);
esc_p5_meta_assert(($noOp['is_default'] ?? false) === true, 'writing exact defaults to absent row remains default');
esc_p5_meta_assert($GLOBALS['sc_test_queries'] === [], 'exact default metadata write is idempotent without storage row');

$GLOBALS['sc_test_queries'] = [];
$GLOBALS['sc_test_read_queries'] = [];
$GLOBALS['sc_test_result_queue'] = [esc_p5_meta_actor(), esc_p5_meta_definition(), []];
$stored = $service->upsert(61, [
    'show_in_summary' => true,
    'filterable' => true,
    'sortable' => true,
    'exportable' => true,
...
esc_p5_meta_assert(($stored['is_default'] ?? true) === false, 'non-default metadata is persisted');
esc_p5_meta_assert(count($GLOBALS['sc_test_queries']) === 1, 'non-default metadata performs one atomic mutation');
$upsertSql = (string) ($GLOBALS['sc_test_queries'][0] ?? '');
esc_p5_meta_assert(str_contains($upsertSql, 'INSERT INTO wp_safecontracts_custom_field_metadata'), 'metadata writes only dedicated table');
esc_p5_meta_assert(str_contains($upsertSql, 'FROM wp_safecontracts_custom_field_definitions d'), 'metadata write derives from current definition');
esc_p5_meta_assert(str_contains($upsertSql, "d.id = 61 AND d.tenant_id = 17 AND d.status = 'active' AND d.data_type = 'text'"), 'metadata write atomically revalidates active tenant definition and data type');
esc_p5_meta_assert(str_contains($upsertSql, 'ON DUPLICATE KEY UPDATE'), 'metadata persistence is atomic upsert');
esc_p5_meta_assert(str_contains($upsertSql, "'Project Field Report'"), 'validated report label is persisted');
...
esc_p5_meta_assert($GLOBALS['sc_test_queries'] === [], 'stored metadata read remains read-only');
...
esc_p5_meta_assert($GLOBALS['sc_test_queries'] === [], 'exact stored metadata write is idempotent');

$GLOBALS['sc_test_queries'] = [];
$GLOBALS['sc_test_result_queue'] = [esc_p5_meta_actor(), []];
esc_p5_meta_throws(static fn () => $service->upsert(999, ['filterable' => true]), InvalidArgumentException::class, 'foreign definition cannot receive metadata');
esc_p5_meta_assert($GLOBALS['sc_test_queries'] === [], 'foreign definition rejection performs no mutation');
...
esc_p5_meta_assert($GLOBALS['sc_test_queries'] === [], 'inactive definition rejection performs no mutation');
...
esc_p5_meta_assert(str_contains($numericSql, "d.data_type = 'decimal'"), 'numeric metadata atomic guard locks immutable decimal type');
esc_p5_meta_assert(str_contains($numericSql, "'measure', 'sum'"), 'numeric measure aggregation metadata persists declaratively');
...
esc_p5_meta_assert(str_contains($resetSql, 'DELETE m FROM wp_safecontracts_custom_field_metadata m'), 'reset removes only metadata override row');
...
esc_p5_meta_assert(str_contains($resetSql, "m.tenant_id = 17 AND m.definition_id = 61 AND d.status = 'active' AND d.data_type = 'text'"), 'reset is tenant/definition/type guarded');
...
$GLOBALS['sc_test_queries'] = [];
$GLOBALS['sc_test_read_queries'] = [];
esc_p5_meta_throws(static fn () => $service->upsert(61, ['filterable' => true]), DomainException::class, 'metadata mutation requires MANAGE_REFERENCE_DATA');
esc_p5_meta_assert($GLOBALS['sc_test_queries'] === [] && $GLOBALS['sc_test_read_queries'] === [], 'global mutation denial occurs before data access');
...
esc_p5_meta_assert(str_contains($serviceSource, 'TenantAuthorization::allowsCapability'), 'metadata service enforces tenant-role capability ceiling');
...
esc_p5_meta_assert(! str_contains($migrationSource, 'safecontracts_contract_template_version_fields'), 'P5-005 schema does not alter P5-004 Template snapshots');
...
esc_p5_meta_assert(! str_contains($policySource, 'eval(') && ! str_contains($policySource, 'exec(') && ! str_contains($policySource, 'expression'), 'metadata policy introduces no executable expression engine');
esc_p5_meta_assert(! str_contains($serviceSource, 'SELECT ') && ! str_contains($serviceSource, 'export'), 'metadata service does not execute reporting queries/exports');

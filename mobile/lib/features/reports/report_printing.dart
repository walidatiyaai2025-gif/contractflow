...
Future<void> showReportOutputDialog(
    ...
    builder: (dialogContext) => AlertDialog(
      ...
      actions: [
        _FormatButton(
          ...
          label: 'PDF',
          onPressed: () => Navigator.pop(dialogContext, ReportOutputFormat.pdf),
        ),
        _FormatButton(
          icon: Icons.description_outlined,
          label: ar ? 'Word' : 'Word',
          onPressed: () =>
              Navigator.pop(dialogContext, ReportOutputFormat.word),
        ),
        _FormatButton(
          icon: Icons.table_chart_outlined,
          label: 'Excel',
          onPressed: () =>
              Navigator.pop(dialogContext, ReportOutputFormat.excel),
        ),
        TextButton(
  ...
  try {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          ar ? 'تعذر إنشاء الملف: $error' : 'Unable to create output: $error',
        ),
      ),
    );
  }
...
Future<void> _printPdf(ReportGrid report, {required bool ar}) async {
  ...
  document.addPage(
    pw.MultiPage(
      ...
      build: (_) => [
        pw.Text(report.title, style: pw.TextStyle(font: bold, fontSize: 18)),
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"/>''',
  ...
  final buffer = StringBuffer()
    ...
    ..write(
      '<w:p><w:pPr>$direction</w:pPr><w:r><w:rPr><w:b/><w:sz w:val="32"/></w:rPr><w:t>${_xml(report.title)}</w:t></w:r></w:p>',
    );
  if (report.subtitle != null && report.subtitle!.trim().isNotEmpty) {
    buffer.write(
      '<w:p><w:pPr>$direction</w:pPr><w:r><w:t>${_xml(report.subtitle!)}</w:t></w:r></w:p>',
    );
  }
  buffer.write(
  ...
  buffer
    ..write('</w:tbl>')
    ..write(
      '<w:p><w:pPr>$direction</w:pPr><w:r><w:t>${_xml(ar ? 'عدد السجلات: ${report.rows.length}' : 'Rows: ${report.rows.length}')}</w:t></w:r></w:p>',
    )
    ..write(

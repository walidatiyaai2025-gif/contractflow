      ...
      return RefreshIndicator(
        ...
        child: ListView(
          ...
          children: const <Widget>[
            SizedBox(height: 180),
            Center(child: Text('No follow-up items match the authorized filters.')),
          ],
        ),
      );
    }
        ...
        SafeArea(
          ...
          child: Row(
            children: [
              ...
              Expanded(child: Text('Page ${page.page}', textAlign: TextAlign.center)),
              IconButton(
                tooltip: 'Next page',
...
final class _FollowUpHistoryScreenState extends State<FollowUpHistoryScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final history = await widget.repository.loadHistory(widget.paymentId, perPage: widget.pageSize);
      if (!mounted) return;
      setState(() {
        _history = history;
        _loading = false;
  ...
  Future<void> _record() async {
    final value = await showDialog<_FollowUpInput>(context: context, builder: (_) => const _FollowUpDialog());
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Follow-up #${receipt.id} recorded.')));
      await _load();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          if (widget.canManage)
            IconButton(tooltip: 'Add follow-up', onPressed: () => unawaited(_record()), icon: const Icon(Icons.add_comment_outlined)),
        ],
      ),
      body: _loading
                          ...
                          return Card(
                            child: Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  Text(item.state, style: Theme.of(context).textTheme.titleMedium),
                                  Text(item.createdAt),
                                  if (item.promisedDate != null) Text('Promised: ${item.promisedDate}'),
                                  if (item.deferredUntil != null) Text('Deferred until: ${item.deferredUntil}'),
                                  if (item.note != null) ...[
...
final class _FollowUpInput {
  const _FollowUpInput({required this.operation, this.note, this.promisedDate, this.deferredUntil});
...
final class _FollowUpDialogState extends State<_FollowUpDialog> {
  ...
  bool get _needsNote => _operation == 'note' || _operation == 'issue' || _operation == 'escalate';
        ...
        child: Column(
          ...
          children: [
            DropdownButtonFormField<String>(
              ...
              items: _operations.map((operation) => DropdownMenuItem<String>(value: operation, child: Text(operation))).toList(growable: false),
              onChanged: (value) {
                if (value != null) setState(() { _operation = value; _error = null; });
              },
            ),
            if (_needsDate)
              TextField(
                controller: _date,
                decoration: InputDecoration(labelText: _operation == 'promise' ? 'Promised date YYYY-MM-DD' : 'Deferred until YYYY-MM-DD'),
              ),
            TextField(
              controller: _note,
              maxLength: 5000,
              minLines: 2,
              maxLines: 5,
              decoration: InputDecoration(labelText: _needsNote ? 'Note' : 'Note (optional)'),
            ),

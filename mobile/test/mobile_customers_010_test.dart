...
void main() {
  test(
    'SC-P9-010 loads bounded customer page with server sort metadata',
    () async {
  ...
  test(
    'SC-P9-010 customer detail uses direct scoped endpoint and safe fields',
    () async {
      final transport = FakeApiTransport(_customerHandler);
      final controller = CustomersController(
        repository: CustomersRepository(_client(transport)),
  ...
  test(
    'SC-P9-010 unauthorized controller fails before network access',
    () async {
      final transport = FakeApiTransport(_customerHandler);
      final controller = CustomersController(
        repository: CustomersRepository(_client(transport)),

...
void main() {
  ...
  test('SC-P9-019 follow-up queue history and actions stay server-backed', () async {
    final transport = FakeApiTransport(_handler);
    final repository = MobileOperationsRepository(_client(transport));

    final queue = await repository.followUpQueue(
      const DashboardFilters(contractId: 11, status: 'overdue'),
      pageSize: 25,
    );
    final history = await repository.followUpHistory(21, pageSize: 25);
    final id = await repository.recordFollowUp(
      paymentId: 21,
      action: 'promise',
    ...
    expect(queue.single.followUpState, 'contacted');
    expect(history.single.state, 'contacted');
    expect(id, 91);
    final actionRequest = transport.requests.last;
    expect(actionRequest.method, 'POST');
    expect(actionRequest.uri.path, endsWith('/payments/21/followups/action'));
    expect(
      jsonDecode(actionRequest.body!) as Map<String, dynamic>,
      <String, dynamic>{
        'action': 'promise',
        'note': 'Customer confirmed',
        'date': '2026-08-25',
...
ApiTransportResponse _handler(Uri uri) {
  if (uri.path.endsWith('/customers/7')) return _ok(_customer());
  if (uri.path.endsWith('/customers')) return _ok(<Object?>[_customer()]);
  if (uri.path.endsWith('/contracts/11/light-edit')) {
    return _ok(<String, Object?>{'contract_id': 11});
  }
  if (uri.path.endsWith('/contracts/11')) return _ok(_contract());
  if (uri.path.endsWith('/contracts')) return _ok(<Object?>[_contract()]);
  ...
  if (uri.path.endsWith('/payments/21/followups/action')) {
    return _ok(<String, Object?>{'followup_id': 91, 'payment_id': 21});
  }
  ...
  if (uri.path.endsWith('/payments')) return _ok(<Object?>[_payment()]);

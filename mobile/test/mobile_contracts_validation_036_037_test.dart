...
void main() {
  group('SC-P9-036 contracts list validation', () {
    test('keeps opaque server status while requiring exact money/date fields', () {
      ...
      expect(
        () => SafeContractsContract.fromData(
          <String, Object?>{..._contractData(), 'base_value': 1000.5},
        ),
        throwsFormatException,
      );
      expect(
        () => SafeContractsContract.fromData(
          <String, Object?>{..._contractData(), 'start_date': '2026-02-31'},
        ),
        throwsFormatException,
      );
    });
    ...
    test('fresh direct read preserves server values and maps 403/404', () async {
      final harness = SafeContractsTestHarness((uri) {
        ...
        if (uri.path.endsWith('/contracts/72')) {
          return SafeContractsTestHarness.error(403, 'forbidden', 'Forbidden');
        }

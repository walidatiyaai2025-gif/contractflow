...
void main() {
  group('SC-P9-009 mobile Excel export', () {
    test('downloads server workbook with the current dashboard filters',
        () async {
    ...
    test('does not call the export endpoint when capability is unavailable',
        () async {
      final transport = FakeApiTransport(_exportHandler);
      final controller = MobileExcelExportController(
        repository: MobileExcelExportRepository(_client(transport)),
    ...
    test('rejects non-XLSX payloads instead of saving arbitrary content',
        () async {
      expect(

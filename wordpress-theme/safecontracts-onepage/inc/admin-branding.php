<?php
/**
 * SafeContracts visual identity for wp-admin and wp-login.
 *
...
/**
 * Replace the WordPress logo item with a SafeContracts home shortcut.
 *
...
function safecontracts_brand_admin_bar( $wp_admin_bar ) {
	...
	$wp_admin_bar->add_node(
		array(
			...
			'title' => '<span class="sc-adminbar-brand-mark" aria-hidden="true"></span><span class="sc-adminbar-brand-text">SafeContracts</span>',
			'href'  => admin_url(),
			'meta'  => array(
				'class' => 'safecontracts-adminbar-brand',
				'title' => 'SafeContracts Dashboard',
			),
		)
	);
}
...
function safecontracts_admin_footer_text() {
	return '<strong>SafeContracts</strong> · Smart & Secure Contract Management';
}
add_filter( 'admin_footer_text', 'safecontracts_admin_footer_text' );

/**
 * Add a focused SafeContracts dashboard card without removing core widgets.
 *
 * @return void
 */
...
/**
 * Render SafeContracts dashboard overview.
 *
 * @return void
 */
function safecontracts_render_dashboard_widget() {
	...
	<div class="sc-dashboard-welcome" dir="<?php echo $is_arabic ? 'rtl' : 'ltr'; ?>">
		<div class="sc-dashboard-welcome-head">
			<span class="sc-dashboard-shield" aria-hidden="true"></span>
			<div>
		...
		<p><?php echo esc_html( $is_arabic ? 'واجهة الإدارة تستخدم نفس الهوية البصرية للواجهة العامة مع الحفاظ على وظائف ووردبريس والنظام كما هي.' : 'The admin workspace now uses the same visual identity as the public site while preserving WordPress and SafeContracts functionality.' ); ?></p>
